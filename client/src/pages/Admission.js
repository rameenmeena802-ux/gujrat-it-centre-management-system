    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md no-print">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <input type="text" name="rollNo" placeholder="Roll No (e.g. IT-004)" value={formData.rollNo} onChange={handleChange} required className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]" />
          ...
          <input type="text" name="contact" placeholder="Contact Number" value={formData.contact} onChange={handleChange} required className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]" />
          ...
          <select name="classTiming" value={formData.classTiming} onChange={handleChange} className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]">
          ...
          <input type="number" name="admissionFee" placeholder="Admission Fee (Rs.)" value={formData.admissionFee} onChange={handleChange} className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]" />
          <input type="number" name="paidFee" placeholder="Paid Fee (Rs.)" value={formData.paidFee} onChange={handleChange} required className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]" />
          <select name="status" value={formData.status} onChange={handleChange} className="border border-gray-300 p-3 rounded-lg focus:ring-2 focus:ring-[#1e3a8a]">
            ...
            <table className="w-full text-left text-[10px] excel-table">
              <thead>
                <tr className="bg-gray-200 text-gray-800">
                  <th className="p-1 border border-gray-400 text-center font-bold">S.No</th>
                  <th className="p-1 border border-gray-400 font-bold">Roll No</th>
                  <th className="p-1 border border-gray-400 font-bold">Student Name</th>
                  <th className="p-1 border border-gray-400 font-bold">Father Name</th>
                  <th className="p-1 border border-gray-400 font-bold">Contact</th>
                  <th className="p-1 border border-gray-400 font-bold">Course</th>
                  <th className="p-1 border border-gray-400 font-bold">Mode</th>
                  <th className="p-1 border border-gray-400 font-bold">Timing</th>
                  <th className="p-1 border border-gray-400 font-bold">Total Fee</th>
                  <th className="p-1 border border-gray-400 font-bold">Admission</th>
                  <th className="p-1 border border-gray-400 font-bold">Paid</th>
                  <th className="p-1 border border-gray-400 font-bold">Remaining</th>
                  <th className="p-1 border border-gray-400 font-bold">Status</th>
                  <th className="p-1 border border-gray-400 text-center font-bold no-print">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredStudents.length === 0 ? (
                  ...
                  filteredStudents.map((s, index) => (
                    <tr key={s._id} className="hover:bg-gray-50">
                      ...
                      <td className="p-1 border border-gray-400 font-semibold text-[#1e3a8a]">{s.rollNo}</td>
                      <td className="p-1 border border-gray-400 font-semibold">{s.studentName}</td>
                      <td className="p-1 border border-gray-400">{s.fatherName}</td>
                      <td className="p-1 border border-gray-400">{s.contact}</td>
                      <td className="p-1 border border-gray-400">{s.course}</td>
                      <td className="p-1 border border-gray-400">
                        <span className={`px-1 py-0.5 rounded text-[10px] font-semibold ${classModeColors[s.classMode]}`}>{s.classMode}</span>
                      </td>
                      <td className="p-1 border border-gray-400">
                        <span className={`px-1 py-0.5 rounded text-[10px] font-semibold ${timingColors[s.classTiming]}`}>{s.classTiming}</span>
                      </td>
                      <td className="p-1 border border-gray-400">Rs. {Number(s.totalFee).toLocaleString()}</td>
                      <td className="p-1 border border-gray-400 text-purple-600">Rs. {Number(s.admissionFee || 0).toLocaleString()}</td>
                      <td className="p-1 border border-gray-400 text-green-600 font-semibold">Rs. {Number(s.paidFee).toLocaleString()}</td>
                      <td className="p-1 border border-gray-400 text-red-600 font-semibold">Rs. {Number(s.totalFee - s.paidFee).toLocaleString()}</td>
                      <td className="p-1 border border-gray-400">
                        <span className={`px-1 py-0.5 rounded text-[10px] font-semibold ${s.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{s.status}</span>
                      </td>
                      <td className="p-1 border border-gray-400 text-center no-print">
                        <button onClick={() => handleEditClick(s)} className="text-blue-600 hover:underline text-[10px] mr-1">Edit</button>
                        <button onClick={() => handleDelete(s._id)} className="text-red-600 hover:underline text-[10px]">Del</button>
                      </td>
                    </tr>
                  ))
                )}
              ...
              {filteredStudents.length > 0 && (
                <tfoot>
                  <tr className="bg-gray-800 text-white font-bold">
                    <td className="p-1 border border-gray-400" colSpan="8">TOTAL STUDENTS: {filteredStudents.length}</td>
                    <td className="p-1 border border-gray-400">Rs. {filteredStudents.reduce((sum, s) => sum + Number(s.totalFee), 0).toLocaleString()}</td>
                    <td className="p-1 border border-gray-400">Rs. {filteredStudents.reduce((sum, s) => sum + Number(s.admissionFee || 0), 0).toLocaleString()}</td>
                    <td className="p-1 border border-gray-400 text-green-300">Rs. {filteredStudents.reduce((sum, s) => sum + Number(s.paidFee), 0).toLocaleString()}</td>
                    <td className="p-1 border border-gray-400 text-red-300">Rs. {filteredStudents.reduce((sum, s) => sum + (Number(s.totalFee) - Number(s.paidFee)), 0).toLocaleString()}</td>
                    <td className="p-1 border border-gray-400 no-print" colSpan="2"></td>
                  </tr>
                </tfoot>
              )}
            </table>
